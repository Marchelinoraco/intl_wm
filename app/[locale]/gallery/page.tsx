  ...
  return (
    <section className="mx-auto max-w-7xl px-6 py-20 lg:px-10">
      <div className="mb-12 flex items-center gap-4">
        ...
        <h1 className="text-3xl font-black uppercase tracking-tighter text-ink">{t.galleryHeading}</h1>
      </div>
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((item, i) => (
          <Reveal key={item.id} delay={Math.min(i, 6) * 50}>
            <figure className="group overflow-hidden rounded-[1.5rem] bg-surface-2 shadow-sm transition-[transform,box-shadow] duration-300 ease-out hover:-translate-y-1 hover:shadow-xl">
              ...
              {item.title && (
                <figcaption className="bg-surface px-5 py-4 text-[11px] font-black uppercase tracking-widest text-ink-2 line-clamp-2">
                  {item.title}
                </figcaption>
              )}
            </figure>
